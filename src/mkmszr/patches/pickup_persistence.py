"""Runtime-confirmed eight-stage ordinary-pickup persistence.

Implements the canonical descriptor/Fire-translation design for the 84
catalogued ordinary pickup locations across all eight main stages.
Scripted/special stage mechanisms remain outside this bitset by design.
"""
...
def build_capture_helper() -> bytes:
...
def build_restore_scanner() -> bytes:
...
def build_pickup_persistence_payload() -> tuple[bytes, int]:
    init = build_init()
...
PICKUP_PERSISTENCE_PAYLOAD, PICKUP_PERSISTENCE_ACTUAL_CODE_SIZE = build_pickup_persistence_payload()
RESTORE_TRAMPOLINE = build_restore_trampoline()
CAPTURE_HELPER = build_capture_helper()


class PickupPersistencePatch:
    """Install runtime-confirmed ordinary-pickup persistence for all main stages."""

    name = "pickup-persistence"
    ...
    def apply(self, rom: RomImage, context: PatchContext) -> tuple[str, ...]:
        ...
        if (
            ...
            raise PatchError("pickup persistence requires NativePayloadPatch first")

        rom.expect_bytes(PROVEN_PAYLOAD_ROM, PICKUP_PERSISTENCE_PAYLOAD)
        ...
        if CAPTURE_HELPER_ROM + len(CAPTURE_HELPER) > capture_limit:
            raise PatchError("pickup capture helper overlaps descriptor table")
        ...
        if data_end > cave_end:
            raise PatchError("pickup persistence exceeds the confirmed permanent cave")
...
def pickup_persistence_patches() -> tuple[
    SafeStageSelectorPatch,
    ArenaReservationPatch,
    NativePayloadPatch,
    PickupPersistencePatch,
]:
    """Return the complete runtime-confirmed pickup-persistence patch sequence."""
