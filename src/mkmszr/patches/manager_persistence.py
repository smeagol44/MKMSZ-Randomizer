"""Runtime-confirmed manager-level pickup persistence path for Fire.

The report-v37 shared manager architecture is now runtime-confirmed for the Fire
Temple starting Potion while its native item callback remains untouched. This
module is still intentionally bounded to that one pickup; the all-stage
generalization remains a separate research milestone.
"""
...
def build_manager_payload() -> tuple[bytes, int]:
    init = build_init()
    restore = build_manager_restore_scanner()

    if len(init) > RESTORE_OFFSET:
        raise AssertionError("V1 init overlaps manager restore scanner")

    actual_size = RESTORE_OFFSET + len(restore)
    if actual_size > CODE_SIZE:
        raise AssertionError("manager Fire persistence exceeds V1 code region")
...
MANAGER_PAYLOAD, MANAGER_ACTUAL_CODE_SIZE = build_manager_payload()
RESTORE_TRAMPOLINE = build_restore_trampoline()
CAPTURE_HELPER = build_manager_capture_helper()


class ManagerPersistenceFirePatch:
    """Install the runtime-confirmed shared-manager Fire capture/restore hooks."""

    name = "manager-persistence-fire"
    ...
    def apply(self, rom: RomImage, context: PatchContext) -> tuple[str, ...]:
        ...
        if (
            ...
            raise PatchError("manager persistence requires the V1 NativePayloadPatch first")

        rom.expect_bytes(PROVEN_PAYLOAD_ROM, MANAGER_PAYLOAD)
        ...
        if permanent_size > NATIVE_BOOTSTRAP_STUB_CAPACITY:
            raise PatchError("manager persistence exceeds the confirmed permanent cave")
        ...
        # Runtime proof specifically confirmed the item callback remains untouched.
        ...
        return (
            ...
            (
                f"V1 Fire bit {FIRE_STARTING_POTION_BIT} at "
                f"0x{STATE_UNCACHED_BASE + FIRE_BITSET_OFFSET:08X} (KSEG1)"
            ),
        )


def manager_persistence_fire_patches() -> tuple[
    ArenaReservationPatch,
    NativePayloadPatch,
    ManagerPersistenceFirePatch,
]:
    """Return the exact runtime-confirmed manager-level Fire patch sequence."""

    return (
        ArenaReservationPatch(),
        NativePayloadPatch(NativePayloadSpec(payload=MANAGER_PAYLOAD)),
        ManagerPersistenceFirePatch(),
    )
