...
The deterministic **next proof** for a generic importer is a single guarded, disposable **P28/P29 attachment presentation** using only retail N64 Sektor donor poses. Keep the host token-`0x10` two-actor grammar, facing flip, rate and attachment/detach transitions; replace image descriptors through a small explicit pose manifest, then manually validate attach, Turn while attached, Up/Down movement, detach, and re-entry in one bounded route. First perform a read-only owner/companion teardown trace to choose the exact hook and avoid making an unmeasured allocation or leaving a companion linked. Failure to find a convincing N64 donor pose should be recorded as an unresolved visual policy, not repaired with PS1/WIMP art.
