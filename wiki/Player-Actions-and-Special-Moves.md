...
In v83 the raw pre-insertion `0x8001BDA0` changes actor `+0x74` but leaves controller `+0x6E4` on the Ice script, so `0x8004B5A0` replaces that early bind with Ice. In v84, the pre-insertion contextual `0x800304C0` instead sets `+0x6E4` to `+0xD90` and advances the first rocket; `0x8004B5A0` consequently advances the **rocket loop** again. This explains the missing Ice frame independently of list insertion. v84 also writes selector zero, and the child subsequently continues the rocket loop.
