...
| Direction-facing control proof v01 | First opposite-direction press immediately hard-hung | The proof accidentally reused rejected `0x802FCE20` as the current-controller global on the flip-only path; v02 changed only that interpretation to `0x802ECE20` and established the corrected foundation | [Player actions and special moves](Player-Actions-and-Special-Moves) |
| Direction-facing v08 empty Turn animation | Repointing file-`0x87` standing Turn slot `0x03` to its terminator removed the visible Turn sequence but still restarted locomotion | Suppressing the downstream animation/token presentation is too late; action selection itself still interrupts the current locomotion state | [Player actions and special moves](Player-Actions-and-Special-Moves) |
| Direction-facing v09 late Turn-helper suppression | Skipping the player-local standing/crouched Turn helpers still interrupted walking and reset locomotion | The generic action dispatcher has already accepted/installed the Turn action by this point; v10 supersedes this by filtering player states 23/24 before installation | [Player actions and special moves](Player-Actions-and-Special-Moves) |
