...
| Direction-facing control proof v03/v04 | Visible Turn persisted despite gating and then fully no-oping `0x8003D86C` | That routine is not the live player-facing toggle path | [Player actions and special moves](Player-Actions-and-Special-Moves) |
| Direction-facing control proof v05 | Patched ROM `0x2A658` direct Turn toggle branch, but runtime looked unchanged | A second matching player-loop Turn latch/toggle exists at ROM `0x2A85C`; suppressing only the first copy is insufficient for the neutral route tested | [Player actions and special moves](Player-Actions-and-Special-Moves) |
