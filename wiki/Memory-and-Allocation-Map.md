...
|---|---|---|---|
| Main arena after <code>0x801AF820</code> | <code>dynamic</code> | Production moves the reset/base floor to <code>0x801AF820</code>. The base is held at <code>0x800EECD0</code>, live bump cursor at <code>0x80111ECC</code>, and the Static-confirmed bump boundary is <code>0x80290990</code>. Allocator <code>0x8006643C</code> has no internal bounds check. | Current/peak cursor and therefore real headroom vary with stage/resources/lifecycle. The boundary does not make any subrange generically free; expanded reservations require measured runtime margin. |
...
| Exact allocator-profiler runtime observation | <code>proof-only</code> / <code>dynamic</code> | A disposable 1 KiB-reservation proof instrumented the central allocator synchronously and recorded a global peak cursor of <code>0x802857E8</code> during one manually exercised eight-stage selector session, leaving <code>0xB1A8</code> = 45,480 bytes = 44.41 KiB below <code>0x80290990</code>. The recorder survived normal stage changes within the session and reset on hard reset. | Runtime-confirmed only for the exercised proof composition/routes. This is measured headroom, not <code>confirmed-free</code>. Per-stage attribution uses the current-stage field, which is updated before some preload/transition work completes; therefore the global peak is authoritative for this proof, while individual stage labels are lifecycle-qualified. |
