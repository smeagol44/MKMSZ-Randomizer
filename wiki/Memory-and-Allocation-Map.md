...
| <code>rom.proof.control_facing_v05_module</code> | <code>[0x00F72000,0x00F72220)</code> | <code>proof-only</code> | 544-byte v05 direction-facing module backing file <code>0x1A</code> | Implementation/CI-confirmed; runtime Pending | no | Reuses the same proof-only source base as v02 with a smaller module after removing the disproven <code>0x8003D86C</code> Turn helper/trampoline. |
...
| Direction-facing v05 expansion slice | <code>proof-only</code> / reserved-pool suballocation | Build-time allocator assigns <code>[0x801AF820,0x801AFA40)</code>, 544 bytes, inside <code>rdram.production.expansion_pool</code>; file <code>0x1A</code> still enters through KSEG1. | Implementation/CI-confirmed; runtime Pending. The smaller size reflects removal of the disproven standalone-Turn helper while preserving the v02 locomotion helpers. |
