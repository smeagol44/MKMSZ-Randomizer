...
| <code>rom.proof.control_facing_v10_module</code> | <code>[0x00F72000, v10 aligned file end)</code> | <code>proof-only</code> | Direction-facing file <code>0x1A</code> module: v02 base plus v10 pre-dispatch Turn-action helper at source offset <code>+0x270</code> | Runtime-confirmed on the bounded v10 control route | no | Supersedes the v02-only proof extent for the current control baseline. The recovered v10 builder guards the v02 end <code>0x00F72268</code>, appends only into FF bytes, aligns the new end to 16 bytes, and checks the loaded end remains below <code>0x801B3420</code>. This remains proof-only ownership, not reusable free space. |
...
| <code>rdram.production.expansion_pool</code> | <code>[0x1AF820,0x1B3420)</code> | KSEG0 <code>[0x801AF820,0x801B3420)</code>; KSEG1 <code>[0xA01AF820,0xA01B3420)</code> | <code>production</code> | Unassigned MKMSZR native expansion pool | Reserved MKMSZR block | Global reservation; future feature slices are build-time assigned | 16 KiB arena-floor proof Runtime-confirmed across all eight safe stage loads; full-production `TEST 16KB` + `rainbow` composition manually exercised without reported regressions; allocator/bounds Implementation/CI-confirmed | yes | <code>addresses.py</code>, <code>allocations.py</code>, <code>tests/test_expansion_allocations.py</code> | The pool is production-reserved ownership, not generic <code>confirmed-free</code>. No feature currently owns a subrange; future claims require named aligned allocation plus their own composition/runtime evidence. |
...
| Direction-facing v10 expansion slice | <code>proof-only</code> / reserved-pool suballocation | File <code>0x1A</code> still begins at <code>0x801AF820</code>; v10 appends its action-gate helper at cached <code>0x801AFA90</code> (uncached alias used for execution), after the v02 base module. | Runtime-confirmed only for the tested v10 control route. The recovered builder explicitly bounds the aligned file end below <code>0x801B3420</code>. This is a proof suballocation, not a production feature owner; any menu/control integration must receive fresh named production allocation. |
