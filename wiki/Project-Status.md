...
| Direction-facing / facing-lock controls | **Partially Runtime-confirmed proof.** v02 proves auto-flip + stock forward locomotion and held-Turn backpedal, but standalone Turn still fires first. v03/v04 targeted the wrong routine; v05 patched only the first of two direct player-loop Turn toggles and produced no runtime change. v06 patches both direct toggle branches and is Pending runtime. | Still proof-only and outside 1.0 scope. Confirm v06, then broaden inventory/action-state/boss coverage before settings or production integration. See [Player actions and special moves](Player-Actions-and-Special-Moves). |
