...
| Reverse Elbow host/action proof + genuine MKT import branch | v6 established repeatable scheduler/action stability; v08/A1.7 runtime-confirmed genuine SCCOMBO10; Sektor v01-v03 runtime-confirm the genuine five-frame Sektor idle art/loop, while the same forward/crouch/air-forward paths still hang; v04 hung before gameplay after adding a new diagnostic HUD | Runtime-test v05, which keeps v04's live cursor at stock resource +0x2EC but removes the confounding HUD; then resolve the separate one-frame idle-entry artifact |
