...
| Reverse Elbow host/action proof + genuine MKT import branch | v6 established repeatable scheduler/action stability; v08/A1.7 runtime-confirmed genuine SCCOMBO10; Sektor v01-v05 runtime-confirm the genuine five-frame Sektor idle art/loop; v05 proves the native-cursor-at-+0x2EC strategy does not fix the same forward/crouch/air-forward hangs, while v04's pre-game hang came from its new HUD | Runtime-test v06, which removes only the donor animation-rate hook; then continue isolating palette/state ownership and the separate one-frame idle-entry artifact |
