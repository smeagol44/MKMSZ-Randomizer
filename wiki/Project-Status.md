...
| Native box indicator | **Production** | Runtime-confirmed `BOX n OF 4` through the native text path; the broader 1.0 randomizer HUD remains Pending |
...
| Rebranded title screen | **Production beta** | Candidate-B art plus configurable uppercase `<NAME> EDITION` are Runtime-confirmed through the normal production composition. The accepted implementation is data-only; the earlier executable-wrapper allocation is Rejected / failed. See [Native UI and presentation](Native-UI-and-Presentation) |
...
| MKT fighter / Sektor takeover | **Runtime-confirmed proof line.** Current v62 confirms the tested Sektor MKT combo-string behavior after translating donor reaction semantics to MKMSZ-native selectors. v58 Run and v59 completed combo visuals are also Runtime-confirmed on their bounded routes. | Still proof-only. The v62 standalone combo record uses production-owned bootstrap space and must be reallocated for any integration. Player-victim grab/throw reactions, specials, and rare presentation families remain Pending. See [Sub-Zero to Sektor animation mapping](Sub-Zero-to-Sektor-Animation-Mapping) and [MKT to MKMSZ compatibility layer](MKT-to-MKMSZ-Compatibility-Layer) |
| Toasty audio | **Runtime-confirmed proof.** Sound proof v03 plays the audibly confirmed MKT Toasty voice through MKMSZ's native audio path. | Final product trigger/allocation is Pending. See [Sounds and music](Sounds-and-Music) |
| Toasty visual | **Partial proof.** v15 Runtime-confirms genuine Toasty CI8 pixels through a dynamic gameplay-HUD texture slot; v16 is Implementation/static-confirmed for the genuine palette-selector path and is still Pending manual runtime validation. | Final palette/runtime confirmation and production-safe trigger/composition remain Pending. See [Native UI and presentation](Native-UI-and-Presentation) |
...
- **Title:** current production uses the Runtime-confirmed data-only Candidate-B composition. The old executable wrapper that overlapped pickup-persistence ownership remains **Rejected / failed**; proof hashes and integration history remain in [Native UI and presentation](Native-UI-and-Presentation).
- **Sektor:** v61 is **Rejected / failed** because donor reaction selector `0x02` was reused numerically. v62 changes only the affected reactions to MKMSZ-native semantics and is **Runtime-confirmed** on the tested combo route. Full version chronology remains in the existing Sektor/MKT owners.
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v15 confirms the image/texture path; v16 palette binding is not yet runtime-confirmed. Detailed diagnostics remain in the existing audio/UI owners.
