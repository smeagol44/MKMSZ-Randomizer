...
- **Sektor Run — cadence + variable-length script (2026-09-25; Runtime-confirmed):** retail MKT Sektor primary slot `0x4A -> +0x1444` contains only `RBRUN1,3,5,7,9,11`. Static reconciliation established donor rate `6` on the 60-Hz MKT scheduler (~100 ms/pose) versus stock MKMSZ rate `2` on the ~30-Hz gameplay scheduler (~66.7 ms/entry), making target Run rate **3** the cadence-equivalent setting. `MKMSZR_sektor-run-cadence_common-proof_v03.z64` (SHA-256 `b46b283189b4dfd4d4c144f5405b5b4b165953e6897890b3d2065cb43901847e`) was manually confirmed to feel correct with the existing two-cycle six-pose script. `MKMSZR_sektor-run-six-entry_common-proof_v04.z64` (SHA-256 `f805a283180486c144910f9627856a85a5eb633b679d62cdf2dee01e6fd571af`) then moved the loop control immediately after the sixth genuine pose and was manually confirmed to work perfectly. This proves the live MKMSZ Run script need not preserve Sub-Zero's historical 12 visual entries: the importer may emit the donor fighter's actual Run-frame count plus loop control, with cadence translated independently. Physical omission of the six supplemental even-pose assets remains accepted; the `0x4CD68` repack analysis is still a storage result pending bounded runtime regression.
