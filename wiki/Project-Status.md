...
| Toasty visual | **Runtime-confirmed proof.** v38 renders the corrected full 78x85 nine-piece Toasty image perfectly across all eight safe stages. The fix is the stock post-allocation convention: slot-record `+0x08` must hold the true DRAM source row width (32 for side pieces, 64 for center pieces) consumed by `0x8001F7A8` / RDP `SetTextureImage`. | Image correctness is solved. Production-safe allocation, slide/lifetime behavior, audio composition, final uppercut/contact + 4% trigger, and full-production integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v38 is Runtime-confirmed perfect across all eight safe stages and validates the post-v37 source-row-width diagnosis: dynamic slot record `+0x08` must be normalized to 32 for side pieces and 64 for center pieces. The remaining task is production composition, not further image-path research: allocate from production-owned space, combine audio + visual, add slide/lifetime behavior, then add the final uppercut/contact + 4% cosmetic trigger. Detailed evidence is canonical in [Toasty visual research](Toasty-Visual-Research).
