...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path. v25-v31 show allocation-order-sensitive residual artifacts. v32 fixes a real saved-state address bug but not the visible corruption. v33-v35 Runtime-confirm stable slot metadata, stable backing-pointer identity, and unchanged narrow-buffer CI8 bytes on the tested routes. v36 is the current bounded render-path test: the six side pieces keep the same 32-byte-padded payloads but use logical width 32 instead of 7. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 confirms the full 78x85 multi-piece image path; v24 rejects inherited secondary rectangle coordinates. v25-v31 show allocation-order-sensitive residual artifacts. v32 corrects a real saved-slot signed-address alias but leaves the corruption unchanged. v33-v35 then reject saved-slot metadata loss, backing-pointer replacement, and post-load narrow-buffer mutation on the tested routes. v36 now tests the downstream renderer/TMEM boundary by presenting the already-32-byte-padded side rows as logical width 32 rather than width 7. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research).
