...
| Rainbow Sub-Zero outfit | **Runtime-confirmed proof.** `MKMSZR_rainbow-outfit_full-proof_v01.z64` (SHA-256 `4f9aff72c3f81d20e7f3d8f3e59f07cff8f7c6e5a1c0ce81d1a1bbb91ab92060`) ran successfully in the full current production composition, with the user reporting the requested rainbow/regression test worked. The proof cycles 64 precomputed BGR555 palettes while changing only clothing indices `0x21..0x3F`. | Still proof-only: its runtime-block repartition, frame-setup hook, and relocated file-`0x87` palette bank must be integrated/guarded in the normal pipeline before the browser/CLI option is enabled. See [Palette and recoloring](Palette-and-Recoloring) |
...
| Toasty visual | **Partial proof.** v15 Runtime-confirms genuine Toasty CI8 pixels through a dynamic gameplay-HUD texture slot. v16 Runtime-confirms that clone-local palette selector `+0x4C` is live, but also exposes a palette-format mistake: source BGR555 words were passed directly where the runtime renderer expects hardware RGBA5551 TLUT words. v17 corrects only those 512 TLUT bytes and is Implementation/static-confirmed pending manual runtime validation. | Final correctly colored runtime confirmation and production-safe trigger/composition remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v15 confirms the image/texture path. v16 Runtime-confirms the custom palette-selector route but fails visually because source BGR555 palette words were registered directly as hardware TLUT data. v17 converts only the custom TLUT to RGBA5551 and is pending manual runtime validation. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research); donor identification and audio proof chronology are canonical in [Toasty audio research](Toasty-Audio-Research), while MKMSZ host mechanics remain in [Audio system](Sounds-and-Music).
