...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path. v25-v31 show allocation-order-sensitive residual artifacts. v32 fixes a real signed-address alias in the saved slot-state accesses but Runtime-confirms that this was not the visible corruption cause. v33 is the current bounded lifecycle diagnostic: it validates each dynamic slot's ID range, active flag, and width/height every HUD frame and raises `V33 SLOT MUT` on any mismatch. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
