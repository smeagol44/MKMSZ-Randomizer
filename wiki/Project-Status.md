...
| Direction-facing / facing-lock controls | **Runtime-confirmed bounded proof with GAME SETTINGS integration through v06.** `TURN: TOGGLE / LOCK` is Runtime-confirmed in the native GAME SETTINGS flow, defaults to `TOGGLE`, and controls the accepted v10 facing-lock behavior. v06 also Runtime-confirms temporary stock/TOGGLE fallback during the first Scorpion forced-facing encounter while preserving `LOCK` as the selected menu value. The two Temple regressions introduced by v04/v05 are fixed. | Still proof-only and outside 1.0 scope. Earth boss detection, broader encounter/action coverage, COMBOS/SPECIALS/JUMP implementation, and production-pipeline integration remain Pending. See [Player actions and special moves](Player-Actions-and-Special-Moves) and [Native HUD and UI](Native-HUD-and-UI). |
