...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path. v25-v31 show allocation-order-sensitive residual artifacts. v32 fixes a real saved-state address bug but not the visible corruption. v33 Runtime-confirms stable slot ID/range/active/width/height metadata; v34 Runtime-confirms the same corruption without a backing-pointer identity change. v35 is the current bounded diagnostic and checks the six narrow side buffers' actual padded CI8 bytes each HUD frame with independent sum32+xor32 checks. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 confirms the full 78x85 multi-piece image path; v24 rejects inherited secondary rectangle coordinates. v25-v31 show allocation-order-sensitive residual artifacts. v32 corrects a real saved-slot signed-address alias but leaves the corruption unchanged. v33 shows the slot records remain valid/active with expected dimensions; v34 reproduces the same stage artifacts without `V34 PTR MUT`, so backing-pointer replacement/reuse is also rejected on the tested routes. v35 now checks the actual six narrow side-buffer contents with per-frame sum32+xor32 integrity checks. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research).
