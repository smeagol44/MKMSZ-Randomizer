...
| Toasty visual | **Runtime-confirmed image proof; production-layout proof pending runtime.** v38 renders the corrected full 78x85 nine-piece Toasty image perfectly across all eight safe stages. v39 preserves that visual path while packing compositor/init/state/TLUT through the current 15 KiB expansion-pool allocator; it uses 4,208 bytes and leaves `0x2B90` bytes. | v39 is Implementation/static-confirmed pending manual runtime validation. Audio composition, slide/lifetime behavior, final uppercut/contact + 4% trigger, and full-production integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v38 is Runtime-confirmed perfect across all eight safe stages. v39 is the current production-layout proof: native compositor/init/state/TLUT are packed through the 15 KiB expansion pool (4,208 bytes used; `0x2B90` left), while six nonzero CI8 slices remain ROM-backed and three fully transparent slices are zero-filled. v39 still uses a disposable fast-test harness and must pass runtime before its layout is promoted to production ownership. After that, the remaining work is audio + slide/lifetime composition and the final uppercut/contact + 4% cosmetic trigger. Detailed evidence is canonical in [Toasty visual research](Toasty-Visual-Research).
