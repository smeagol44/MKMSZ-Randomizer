...
| GAME SETTINGS controls | **TURN production; future rows Pending.** | `TURN: TOGGLE / LOCK` is Runtime-confirmed in the shared patch core and defaults to `TOGGLE`. The accepted next rows are `COMBOS: CLASSIC / ASSIST` and `SPECIALS: CLASSIC / MODERN`. `JUMP: DPAD / BUTTON` is planned to stay visible but disabled/greyed until **COMBOS=ASSIST AND SPECIALS=MODERN**; only then is `BUTTON` available. Future gameplay semantics remain Pending. Earth boss detection for TURN remains Pending. See [Native HUD and UI](Native-HUD-and-UI). |
