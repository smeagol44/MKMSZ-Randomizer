...
| Direction-facing / facing-lock controls | **Partially Runtime-confirmed proof.** v02 proves opposite-direction auto-flip + stock forward locomotion and held-Turn backpedal behavior, but the standalone Turn still fires first. v01 is Rejected / failed for the wrong current-process pointer; v03/v04 are Rejected / failed because `0x8003D86C` is not the visible player-turn source. Static tracing now identifies the direct player-loop toggle at `0x80029A44..0x80029A70`; v05 skips that toggle with one guarded branch edit and is Pending runtime. | Still proof-only and outside 1.0 scope. Confirm v05, then broaden inventory/action-state/boss coverage before settings or production integration. See [Player actions and special moves](Player-Actions-and-Special-Moves). |
