...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path, but small stage-specific edge corruption remains. v24 rejects inherited secondary rectangle coordinates as the cause. v25-v30 Runtime-confirm that changing only dynamic texture allocation order changes the corruption pattern and affected-stage set; v27/v30 are the strongest bounded arrangements so far, while Fortress can remain clean and Earth can be reduced to 1-2 stray pixels. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. v31 is the remaining bounded left-column allocation permutation. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Sektor:** v61 is **Rejected / failed** because donor reaction selector `0x02` was reused numerically; v62 fixes that combo route. The later missile line is documented through v70: v63 rejects an unsafe file-`0x87` liveness assumption, v65-v67 reject intrusive lifecycle replacements, v68 is the stable stock-lifecycle control, v69 proves the resolved chest-pose cursor hook, and v70 proves genuine rocket presentation while rejecting Ice-path tuning as the final adapter strategy. Full chronology remains canonical in [Sektor takeover proof history](Sektor-Takeover-Proof-History).
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 confirms the full 78x85 multi-piece image path; v24 rejects inherited secondary rectangle coordinates as the remaining defect. v25-v30 then isolate the residual corruption to dynamic texture allocation identity/lifecycle: changing allocation order alone changes which stages/pieces corrupt, with Fortress clean in the stronger arrangements, Earth reduced to 1-2 pixels, and Temple/Fire retaining small allocation-sensitive edge defects. v31 is the final bounded left-column order permutation before further permutations are abandoned in favor of slot-ID/lifecycle instrumentation. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research); donor identification and audio proof chronology are canonical in [Toasty audio research](Toasty-Audio-Research), while MKMSZ host mechanics remain in [Audio system](Sounds-and-Music).
