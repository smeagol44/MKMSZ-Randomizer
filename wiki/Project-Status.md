...
| Toasty audio | **Runtime-confirmed donor playback; dedicated route pending runtime.** Sound proof v03 plays the audibly confirmed MKT Toasty voice through MKMSZ's native audio path. | The v03 pickup-host route is proof-only and must not ship. v46 is the guarded dedicated-audio proof: stock pickup descriptor/event/patch/subpatch/waveform remain byte-identical while Toasty uses isolated event `52` -> patch `68` -> subpatch `608` -> waveform `533`; manual validation is Pending. See [Toasty audio research](Toasty-Audio-Research). |
| Toasty visual | **Runtime-confirmed gameplay trigger and presentation proof.** v38 is perfect across all eight safe stages; v40 validates the compact uncached layout; v42 is the accepted lower-right 3/16/8 presentation baseline. v44's 50% diagnostic Runtime-confirms that the reconciled successful-reaction family triggers the complete effect in gameplay. | Supplied MKT Rev. 2 retail confirms selectors `08/0E/12/13/14/15/17` and the reaction-driven `FX_COMMENT` probability **6.4% normally / 16% on MK2 Pit Star**. MKMSZR's intended normal rate is the user-selected **8%**; that tuning does not need another isolated proof. Production integration is gated on v46's dedicated audio route restoring ordinary pickup sound. See [Toasty visual research](Toasty-Visual-Research) and [Toasty audio research](Toasty-Audio-Research). |
