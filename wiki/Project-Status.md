...
| GAME SETTINGS controls | **TURN production; future rows Pending.** | `TURN: TOGGLE / LOCK` is Runtime-confirmed in the shared patch core and defaults to `TOGGLE`. The accepted next menu row is `COMBOS: CLASSIC / ASSIST`, followed by `SPECIALS: CLASSIC / MODERN`; conditional `JUMP: DPAD / BUTTON` appears only when COMBOS or SPECIALS is non-classic. Future rows are design-approved but gameplay semantics remain Pending. Earth boss detection for TURN remains Pending. See [Native HUD and UI](Native-HUD-and-UI). |
