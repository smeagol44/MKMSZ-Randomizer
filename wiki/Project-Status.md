...
| Toasty visual | **Runtime-confirmed presentation baseline; gameplay-trigger proof pending runtime.** v38 is perfect across all eight safe stages; v40 validates the compact uncached expansion layout; v41 Runtime-confirms composed visual+voice behavior; v42 is the accepted presentation baseline and is Runtime-confirmed perfect at the lower-right edge with 3/16/8 timing. v43 is Implementation/static-confirmed and replaces only the temporary trigger with the reconciled successful-reaction family. | Supplied MKT Rev. 2 retail now confirms selectors `08/0E/12/13/14/15/17` and the reaction-driven `FX_COMMENT` probability of **6.4% normally / 16% on MK2 Pit Star**; the earlier 4% site is a different Toasty-capable path. MKMSZ maps the same reaction family and v43 hooks the successful/unblocked transfer seam at `0x8002DF28`, using native `randper(0x40)`. Manual runtime validation of v43 and dedicated production audio routing/integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) and [Toasty audio research](Toasty-Audio-Research). |
