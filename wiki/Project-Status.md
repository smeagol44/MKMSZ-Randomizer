...
| Toasty visual | **Runtime-confirmed image proof.** v38 renders the corrected full 78x85 nine-piece Toasty image perfectly across all eight safe stages. v39 production-layout attempt is **Rejected / failed**: broad HUD/player/Toasty corruption exposed an invalid cached-KSEG0 execution path for freshly raw-loaded expansion code, and its 4,208-byte slice also carried an unoptimized proof compositor. | Rebuild production layout around compact table-driven code and uncached KSEG1 execution via a bounded static trampoline. Audio composition, slide/lifetime behavior, final uppercut/contact + 4% trigger, and full-production integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03 and visual v38 is Runtime-confirmed perfect across all eight safe stages. v39 is **Rejected / failed**: it executed freshly raw-loaded expansion code through cached KSEG0 aliases, contrary to the established uncached KSEG1 native-payload contract, and it promoted an unnecessarily large unrolled proof compositor. The next production-layout proof must use a compact table-driven compositor/init and a static trampoline into uncached expansion-pool code. Detailed evidence is canonical in [Toasty visual research](Toasty-Visual-Research).
