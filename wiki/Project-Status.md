...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path; v25-v37 retain stage-dependent missing/extra edge pixels. Post-v37 static tracing found a concrete omitted stock step: `0x8001C2B4` may reuse a capacity-compatible dynamic slot without refreshing record `+0x08`, while `0x8001F7A8` consumes `+0x08` as the RDP `SetTextureImage` DRAM source width and stock callers rewrite it after allocation. The Toasty builders never did. | Causal link is a strong inference pending one bounded proof that restores only `record+0x08` to the actual 32/64-byte source row width. Stage-stable rendering and final product integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
