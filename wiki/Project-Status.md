...
| MKT fighter / Sektor takeover | **Runtime-confirmed proof line with missile experiments through v73.** v62 remains the stable tested combo baseline; v69 Runtime-confirms a safe one-frame genuine chest-open player pose, v70 Runtime-confirms a genuine horizontal rocket frame reaching/travelling as the projectile, v72 Runtime-confirms donor-retail placement `(+5,+38)` with correct facing mirroring, and v73 Runtime-confirms stable visible acceleration via projectile actor `+0x14`. Static velocity-unit calibration is now resolved: MKMSZ `0x80017F80` integrates projectile `+0x14` through a `>>8`, orientation transform, and `3x` fixed8 position step at the game's ~30 Hz movement cadence, so faithful MKT 60 Hz acceleration requires temporal resampling rather than raw constant copying. Runtime validation of that resampler, reacting-target speed, smoke/effects, audio, palette/helper cleanup, strike/reaction translation, and final lifecycle composition remain Pending. | Still proof-only. v62 and v65-v70 use proof code inside production-owned bootstrap space and require reallocation for any integration. The accepted special-move direction is now the generic donor→MKMSZ projectile adapter, not further Ice-path tuning. See [Sektor takeover proof history](Sektor-Takeover-Proof-History), [MKT adapter primitives](MKT-Adapter-Primitives), and [MKT compatibility overview](MKT-to-MKMSZ-Compatibility-Layer). |
...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path; v25-v37 retain stage-dependent missing/extra edge pixels. Post-v37 static tracing found a concrete omitted stock step: `0x8001C2B4` may reuse a capacity-compatible dynamic slot without refreshing record `+0x08`, while `0x8001F7A8` consumes `+0x08` as the RDP `SetTextureImage` DRAM source width and stock callers rewrite it after allocation. v38 is Implementation/static-confirmed and restores only that `+0x08` width state to 32 for side rows / 64 for center rows. | Runtime validation of v38 on Temple/Earth/Fire/Fortress is Pending; stage-stable rendering and final product integration remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Sektor:** v61 is **Rejected / failed** because donor reaction selector `0x02` was reused numerically; v62 fixes that combo route. The later missile line is documented through v73: v63 rejects an unsafe file-`0x87` liveness assumption, v65-v67 reject intrusive lifecycle replacements, v68 is the stable stock-lifecycle control, v69 proves the resolved chest-pose cursor hook, v70 proves genuine rocket presentation while rejecting Ice-path tuning as the final adapter strategy, v71 rejects a combined placement/movement revision after an immediate hard hang, v72 Runtime-confirms direct `(+5,+38)` native placement with correct facing mirroring, and v73 Runtime-confirms stable visible acceleration; the subsequent static trace resolves the target integrator/cadence and derives a pairwise 60 Hz donor -> 30 Hz target resampling policy that remains runtime Pending. Full chronology remains canonical in [Sektor takeover proof history](Sektor-Takeover-Proof-History).
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 confirms the full 78x85 multi-piece path. The post-v37 renderer trace supersedes the earlier simple "7-pixel TMEM width" interpretation: dynamic slot record `+0x08` is caller-owned DRAM source-image width, stock callers rewrite it after `0x8001C2B4`, and the allocator reuse path can leave it stale. The Toasty builders omitted that write, which strongly explains both missing edge texels and floating pixels, plus their stage/allocation-order sensitivity. The next proof must change only this field to the actual 32/64-byte source row width. Detailed evidence is canonical in [Toasty visual research](Toasty-Visual-Research).
