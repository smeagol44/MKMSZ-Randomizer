...
| MKT fighter / Sektor takeover | **Runtime-confirmed proof line through v86; v85 built but untested and statically rejected for the ordinary straight branch.** v62 remains the stable combo baseline; v75 remains the stable missile-flight baseline. v80/v83 retain the stock Ice palette and parent frame advance; v81/v82/v84 force selector zero, which is not the dynamically acquired Sektor palette. The ordinary v75 parent wrapper is bypassed; its child later advances the rocket loop. v83 early raw bind is overwritten before flight; v84 moves the controller cursor to the rocket loop before the parent advance but still uses the wrong palette selector. The former same-shape explanation is disproved by `0x8001BDA0`, and v85 suppresses motion/render-list insertion on the normal branch without a reachable reinsertion. v86 acquires the correct resident Sektor palette source dynamically and materially improves the rocket, but the compact rocket asset had already remapped donor indices into grayscale resident TLUT entries, so donor-faithful projectile palette content remains Pending. Helpers, strike/effects/audio, balanced resource lifecycle and final integration remain pending. | Proof-only, including bootstrap allocation overlap. Preserve the generic donor→MKMSZ adapter; see [Sektor takeover proof history](Sektor-Takeover-Proof-History), [MKT adapter primitives](MKT-Adapter-Primitives), and [Player actions and special moves](Player-Actions-and-Special-Moves). |
...
- **Sektor:** v62 remains the tested combo baseline; v75 the last stable tested straight-missile flight. v76–v79 remain negative/failed controls. v80–v84 are runtime observed with distinct palette/frame results, reconciled in [Sektor takeover proof history](Sektor-Takeover-Proof-History) and the host lifecycle owner [Player actions and special moves](Player-Actions-and-Special-Moves). v85 was built but **never runtime-tested**; its deferred insertion is statically invalid on the ordinary straight branch and should not be used as the next proof. v86 is Partial Runtime-confirmed: correct dynamic palette-source ownership improves the rocket substantially, while the remaining mismatch is now traced to the earlier grayscale resident-TLUT approximation rather than selector-zero or active-handle state.
