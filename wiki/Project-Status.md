...
| Direction-facing / facing-lock controls | **Partially Runtime-confirmed proof.** v02 proves opposite-direction auto-flip into stock forward locomotion and held-Turn backpedal/facing-lock behavior after the stock Turn flip. v01 is Rejected / failed for the bad current-process address. v03/v04 targeted the wrong Turn routine; v05/v06 patched one then both statically identified direct Turn-toggle branches, but runtime behavior remained unchanged. | Still proof-only and outside 1.0 scope. Stop speculative Turn patches. Current next step is a live BizHawk trace of writes to the player actor's `+0x8C` facing word and the CPU PC at the write, then patch only the observed live path. See [Player actions and special moves](Player-Actions-and-Special-Moves). |
