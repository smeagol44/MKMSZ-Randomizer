...
| Toasty visual | **Partial proof.** v17 Runtime-confirms the corrected hardware RGBA5551 TLUT; v19-v20 Runtime-confirm multi-node composition, with v20 rendering a coherent full-height 64x85 center crop. Direct 78px-wide v21 is Rejected / failed. v22 localizes the remaining full-width defect to the two 7px side columns; v23 is Implementation/static-confirmed and corrects only their aligned backing-row stride. | Full 78x85 runtime confirmation, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v17 confirms the corrected hardware TLUT, and v20 is the current strongest visual runtime proof: a coherent 64x85 Toasty center crop composed from three HUD slices. v21 direct 78px width is Rejected / failed; v22 shows that the center 64px remains correct while tightly packed 7px side columns corrupt. v23 pads those narrow source rows to the allocator-aligned 32-byte pitch and is pending runtime validation. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research); donor identification and audio proof chronology are canonical in [Toasty audio research](Toasty-Audio-Research), while MKMSZ host mechanics remain in [Audio system](Sounds-and-Music).
