...
| Toasty visual | **Runtime-confirmed image, compact layout, and feature core.** v38 is perfect across all eight safe stages; v40 validates the compact uncached expansion layout; v41 Runtime-confirms composed visual+voice behavior; v42 is the accepted presentation baseline and is Runtime-confirmed perfect at the lower-right edge with 3/16/8 timing. | Dedicated production audio routing and the final gameplay trigger remain Pending. Donor-source reconciliation shows the trigger is **not uppercut-only**: `FX_COMMENT` is reached by uppercut plus several combo-ending reaction paths. Exact supplied-retail reaction classes/probability must be reconciled before the next ROM. See [Toasty visual research](Toasty-Visual-Research) and [Toasty audio research](Toasty-Audio-Research). |
| Direction-facing / facing-lock controls | **Runtime-confirmed bounded proof through v10.** Left/Right without Turn is world-direction movement: opposite input immediately flips facing and continues through normal forward locomotion. Held Turn locks facing and preserves vanilla backward movement; releasing Turn while still holding direction immediately returns to auto-facing/forward behavior. Turn alone has no visible turn, and rapid Turn taps while walking no longer restart locomotion. Inventory Combine remains on the normal Turn/Combine semantic bit. v08/v09 are superseded intermediate proofs; v10 fixes the interruption at the correct boundary by suppressing the player-only standing/crouched Turn actions (states 23/24) before generic action installation. | Still proof-only and outside 1.0 scope. Randomizer-settings integration, broader action/boss coverage, and production-pipeline integration remain Pending. See [Player actions and special moves](Player-Actions-and-Special-Moves). |
