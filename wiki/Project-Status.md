...
| Toasty visual | **Partial proof.** v17 Runtime-confirms the corrected hardware RGBA5551 TLUT; v19-v20 Runtime-confirm multi-node composition. v23 Runtime-confirms a coherent full 78x85 image on most tested stages after 7px side-row stride correction, but Fire and Earth retain small stage-specific edge corruption. v24 normalizes the cloned node's secondary rectangle coordinates and is **Rejected / failed** for that hypothesis because Fire/Earth reproduce the exact same corruption. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 now Runtime-confirms a coherent full 78x85 Toasty on most tested stages after allocator-aligned side-row padding; Fire and Earth still show repeatable small edge corruption while Temple/other tested stages, including Prison/Fortress lifecycle checks, render cleanly. v24 changed only the cloned node's secondary rectangle coordinates (`+0x28/+0x2A/+0x38/+0x3A`) and reproduced the exact same Fire/Earth corruption, rejecting that inherited-coordinate hypothesis. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research); donor identification and audio proof chronology are canonical in [Toasty audio research](Toasty-Audio-Research), while MKMSZ host mechanics remain in [Audio system](Sounds-and-Music).
