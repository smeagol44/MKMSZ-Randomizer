...
| Direction-facing / facing-lock controls | **Partially Runtime-confirmed proof.** v02 proves opposite-direction auto-flip + stock forward locomotion and the held-Turn backpedal behavior, but follow-up testing found Turn still performs one stock standalone turn before becoming the modifier. v01 remains Rejected / failed for the wrong `0x802FCE20` pointer. v03 replaces only the Turn gate with player-actor identity via persistent player controller `0x802C1AC0`; runtime validation is Pending. | Still proof-only and outside 1.0 scope. Confirm v03 standalone-Turn suppression, then broaden action-state/inventory/boss coverage before any settings or production-pipeline integration. See [Player actions and special moves](Player-Actions-and-Special-Moves). |
