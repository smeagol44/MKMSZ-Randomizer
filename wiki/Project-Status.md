...
| Toasty visual | **Partial proof.** v23 proves the full 78x85 nine-piece image path. v25-v31 show allocation-order-sensitive residual artifacts. v32 fixes a real signed-address alias in the saved slot-state accesses but Runtime-confirms that this was not the visible corruption cause. v33 Runtime-confirms the same corruption with no slot-ID/range/active/width/height invariant failure. v34 is the current bounded diagnostic and checks whether each saved slot's backing pointer in `0x800ED940[id]` changes after load. | Stage-stable full-image rendering, final placement/animation/trigger, audio composition, and production-safe allocation remain Pending. See [Toasty visual research](Toasty-Visual-Research) |
...
- **Toasty:** audio identification/playback is Runtime-confirmed in v03. Visual v23 confirms the full 78x85 multi-piece image path; v24 rejects inherited secondary rectangle coordinates. v25-v31 show allocation-order-sensitive residual artifacts. v32 corrects a real saved-slot signed-address alias but leaves the corruption unchanged. v33 adds per-frame checks for saved slot ID range, active state, and requested width/height; the same corruption reproduced without `V33 SLOT MUT`, so those slot-record invariants are stable on the tested routes. v34 now snapshots and checks the backing pointer `0x800ED940[id]`. Detailed visual diagnostics are canonical in [Toasty visual research](Toasty-Visual-Research).
