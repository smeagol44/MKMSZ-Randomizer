# Native UI and presentation — superseded index

This mixed page was split by Wiki refactor Audit Task 10.

- Gameplay HUD hooks, native text, render-node usage, the box indicator, generic renderer conclusions, and 1.0 HUD reference material now belong to [Native HUD and UI](Native-HUD-and-UI).
- Legal/boot presentation, title-screen art, configurable edition text, boot phrase presentation, branding layout, and visual acceptance now belong to [Presentation and branding](Presentation-and-Branding).
- Stable render-node structure grammar remains canonical in [Data structures and encodings](Data-Structures-and-Encodings).

## Temporary Toasty visual owner

**Task 11 has not been performed.** To preserve link stability and avoid moving the full diagnostic chronology early, the existing Toasty visual material remains below on this old slug. Some intermediate conclusions are intentionally superseded by later entries; [Runtime validation status](Runtime-Validation-Status) carries the concise current cross-domain evidence summary. Task 11 will extract and normalize this material into the dedicated Toasty visual owner.
