...
**Evidence:** disassembly of clean MKT USA Rev. 2 (`30efdbe2…`) and MKMSZ USA Rev. 0 (`9c18254a…`), plus the exact v75 proof (`e77b334f…`) and v76–v79 builders. Addresses in the left column are **MKT retail VAs**; those in the right column are **MKMSZ VAs**. This is static evidence, except where the v75 route is explicitly called Runtime-confirmed. MKT's source names below are resolved against the retail call chain; source-space constants must not replace retail N64 values.
...
| Create projectile and first descriptor | `make_rocket` `0x8007C00C` calls `0x8007F7EC`/`0x8007F014` to create the projectile object; `rocket1_proc` `0x8007C100` selects robot animation `0x43`, part 4 via `0x8000D1F4`, then sets the projectile visual/animation before flight. | Native token `0x0B -> 0x80030974 -> 0x80034510 -> 0x800281A0/0x80028128` stages a resource actor at controller `+0x714` and `+0x648` and inserts it. v75's parent wrapper `0x8009A210` substitutes the imported rocket at `actor+0x98+0xD90` and advances a frame **after the token has inserted its initially Ice-derived actor but before child creation**. The child `0x8004B82C` subsequently sets an Ice cursor from `0x80291C10+0xFD0` and calls one-time `0x8004CC14` setup. The inserted actor can thus first display an Ice-derived frame before the parent swaps it; the exact render tick of the observed flash is not statically proven. The true first-render descriptor must be bound **before** actor insertion/first display, while keeping the token's staging transaction intact. |
...
| Palette | Donor `0x8007F75C` binds a projectile-local palette via `0x80001730` with `0x80156A70` (rocket palette source); explosion also uses its own effect resource. | v75's first-frame wrapper **writes selector `actor+0x9E=0`**, then `0x800304C0` binds the rocket descriptor/active handle. The existing converted `ROCKET_P` indices reside in Sektor's TLUT, but resource conversion alone does not bind that palette to a projectile. `+0x80` active handle and `+0x9E` selector lifetime must be checked through first display, flight and destruction. Owner-selector copying in v79 has no isolated runtime validation. |
...
**Adapter contract:** `begin_special(owner)`; `create_resource_actor(rocket_entry)` with a valid first descriptor and palette handle **before first render**; `bind_origin(owner, actor, donor_anchor_semantic)` without calling an unqualified `0x80031394`; `place_local(actor,+5,+38)`; `start_child(actor, owner/opponent, rocket_program)` through the `+0x714 -> +0x6E0` transfer; `flight_tick(child_state)` with paired donor cadence; `strike(semantic_record)` and explicit `on_hit/on_offscreen`; `spawn_smoke/explosion` with independent effect ownership; `end_child`; `end_parent` after chest recovery. The missing generic pieces are a safe first-render/origin constructor, effect and palette lifetime binding, explicit child scratch/state, donor strike/reaction translation, and the independent parent animation phase runner. Preserve stock parent/companion lifecycle in the next bounded proof until these contracts are established. The first-render seam precedes child creation: it is the token `0x0B` resource actor constructor/insertion, **not** merely the child entry. Trace and guard the exact resource entry consumed by `0x80034510` and its other references before any proof changes it.

**Candidate constructor operand (Static-confirmed bytes, live route Pending):** exact v75 fighter file `0x87` has three `0x0B,0x00034A58` pairs at file-relative `+0xE00/+0xE04`, `+0xE50/+0xE54`, and `+0xEAC/+0xEB0`. The operand leads to a valid resource entry `+0x34A58 -> +0x34A60` for an Ice-derived descriptor. Imported rocket `+0x4E6E4 -> +0x4E6EC` uses the same outer-entry/descriptor grammar. The actual token consumed by the straight-missile parent and any other users of these three pairs must still be established; changing all three on appearance alone would be another uncontrolled composition.
