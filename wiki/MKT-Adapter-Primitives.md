...
**Adapter contract:** `begin_special(owner)`; `create_resource_actor(rocket_entry, projectile_palette)` as one guarded pre-insertion transaction in which descriptor selection, selector allocation/store, native frame setup, and active handle `+0x80` are coherent; `bind_origin(owner, actor, donor_anchor_semantic)` without calling an unqualified `0x80031394`; `place_local(actor,+5,+38)`; `start_child(actor, owner/opponent, rocket_program)` through the `+0x714 -> +0x6E0` transfer; `flight_tick(child_state)` with paired donor cadence; `strike(semantic_record)` and explicit `on_hit/on_offscreen`; `spawn_smoke/explosion` with independent effect ownership; `end_child`; `end_parent` after chest recovery. The constructor operand is now resolved, but v80 shows descriptor replacement alone is insufficient. v81 validates that the constructor and pre-insertion selector are part of the correct first-render boundary, but it also exposes the missing **selector → active-handle rebind**. Static trace of `0x800281A0` shows that it calls `0x8001BDA0` at `0x80028254` during construction, before token handler `0x80030974` later allocates/stores its stock selector. `0x8001BDA0` resolves actor `+0x9E` through the palette table and writes the resulting active handle to actor `+0x80`. Therefore changing `+0x9E` after constructor-time frame setup can remove the visible fallback while leaving `+0x80` incoherent. The next bounded proof should preserve the stock palette allocation/refcount lifecycle, keep the rocket constructor operand, then rerun native frame setup on the already-selected rocket shape **after** the stock selector store and **before insertion**, so `+0x9E/+0x80` become coherent without inventing a palette or suppressing cleanup. After that, the next semantic step is replacing the stock Ice palette source with donor rocket palette ownership; placement/origin remains separate and should not be cosmetically retuned.
