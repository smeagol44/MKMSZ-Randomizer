...
**Reconciled adapter contract (v80–v85).** Keep the layering `MKT move semantics → generic donor→MKMSZ adapter primitives → MKMSZ host ABI`. `create_projectile` acquires its own frame resource, fixed/dynamic texture slot, source palette handle and selector; `bind_frame` runs after actor context is initialized and requests/commits render-record decode and texture upload; `place_local` uses the confirmed `(+5,+38)` translation without assuming `0x80031394` is safe; `handoff_child` preserves `+0x714 -> +0x6E0` and parent `+0x648` clearing; `flight_tick` preserves the v75 cadence/velocity and separately translates strike, effect, audio and cleanup semantics. Actor-list insertion is part of update and render eligibility, not a substitute for frame binding or resource acquisition.
...
A single bounded future palette-source proof is now supported: start from exact v75; at the token's `0x8001C528` acquisition, select the already resident **owner type-4 source at file-`0x87 +0x4584C`**, guarded to the intended action/owner, retaining the native returned handle-minus-`0x80` store. Do not force selector zero, change timing, suppress insertion/helpers, or alter v75 velocity. This isolates whether the rocket indices render coherently through the correct dynamic Sektor palette. The initial Ice-derived frame may remain, and this proof alone would not validate donor impact or balanced resource release. No ROM is built by this reconciliation.
