...
| Palette | Donor `0x8007F75C` binds a projectile-local palette via `0x80001730` with `0x80156A70` (rocket palette source); explosion also uses its own effect resource. | After `0x80034510` returns the new actor, `0x80030974` calls `0x8001C528(0x800B1A24,0x100,0)` and stores `(return - 0x80)` to new actor `+0x9E` **before actor insertion**. This fixed stock record is independent of the constructor resource operand, explaining why v80's descriptor-only change could leave the first visible projectile Ice-blue. v75 later overwrites `actor+0x9E=0` in its parent first-frame wrapper, after insertion. v81 keeps the stock allocation/refcount call but, as a **disposable diagnostic**, forces the token-`0x0B` pre-insertion selector store to `0` while also using the live `+0xDB8/+0xDBC` rocket constructor operand. **Runtime-confirmed partial result:** the one-frame blue Sektor-like projectile fallback disappears, proving this is the relevant first-display selector seam; however, the travelling rocket becomes visibly palette/graphics-corrupted. Frame-by-frame review shows the projectile remains a compact horizontal 39x9-class silhouette but only sparse bright pixels survive, consistent with a palette/active-handle mismatch rather than a new geometry/placement failure. This store override is intentionally broader than the eventual generic adapter and is not a production implementation. |
...
**Adapter contract:** `begin_special(owner)`; create/stage the projectile actor without exposing an invalid intermediate frame; bind the donor descriptor/palette through a coherent native animation transaction; publish/insert the actor only after the first donor frame is valid; `bind_origin(owner, actor, donor_anchor_semantic)` without calling an unqualified `0x80031394`; `place_local(actor,+5,+38)`; `start_child(actor, owner/opponent, rocket_program)` through the `+0x714 -> +0x6E0` transfer; `flight_tick(child_state)` with paired donor cadence; `strike(semantic_record)` and explicit `on_hit/on_offscreen`; `spawn_smoke/explosion` with independent effect ownership; `end_child`; `end_parent` after chest recovery. The constructor operand is now resolved, but v80 shows descriptor replacement alone is insufficient. v81 proves that early constructor-time rocket substitution can remove the one-frame Ice/Sektor projectile fallback, but the resulting rocket is graphically corrupted. v82 explicitly rebinds `+0x80` from selector 0 and produces **no visible change**, so stale active handle alone is rejected. The stronger static distinction is ordering: `0x800281A0` calls `0x8001BDA0` at `0x80028254` before token handler `0x80030974` assigns the secondary actor's `+0x9C`, owner-derived `+0x78/+0x98`, position/flags and final palette selector, then inserts at `0x80030AC8`. Exact v75 instead constructs the stock Ice shape first and later performs an Ice→rocket shape transition in its parent wrapper. Because `0x8001BDA0` explicitly compares old actor `+0x74` against the requested shape, constructor-time rocket substitution makes the later v75 rocket bind a **same-shape** update rather than the full shape transition v75 originally exercised. v83 kept the stock constructor/palette path and bound the genuine rocket after token initialization, immediately before insertion, but runtime showed the same first Ice/Sektor projectile frame and Ice-blue missile. This rejects raw `0x8001BDA0` as a sufficient first-render adapter primitive at that seam. The static difference from the known-good later v75 bind is controller context: at `0x8004B040` the projectile is current at `+0x6E0`, `+0x6E4` is redirected to `actor+0x98+0xD90`, selector 0 is installed, and the **full `0x800304C0` animation interpreter** advances the frame. v84 reuses that complete transaction before insertion and **Runtime-confirms first-visible ownership**: the one-frame Ice/Sektor fallback disappears. The rocket still renders with the v81-style sparse/glitched texture, however. The common property of v81/v84 is that ROCKETD1 becomes actor `+0x74` before the later v75 wrapper, so the original full Ice→rocket transition is no longer exercised there. The stronger adapter conclusion is therefore to separate **construction/staging visibility** from **descriptor binding**. Static trace of `0x80024650` shows it only links the actor into the active list, ordered by `+0x5C` and then `+0x30`; it does not initialize render state. v85 keeps the staged actor off-list, leaves its stock Ice descriptor intact, allows placement and owner staging to complete, performs the original Runtime-confirmed v75 Ice→rocket animation transaction at the original point, and only then links the actor into the active list. This deferred-visibility boundary is the current bounded first-render candidate. After that, the next semantic step is replacing the stock Ice palette source with donor rocket palette ownership; placement/origin remains separate and should not be cosmetically retuned.
