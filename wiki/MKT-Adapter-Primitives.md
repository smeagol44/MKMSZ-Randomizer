...
**Evidence:** clean MKT Rev. 2 and MKMSZ Rev. 0, exact Runtime-confirmed v75 (`e77b334f…`), and v80–v85 builders, with v80–v84 observations supplied by the user. The exact normal-flight host lifecycle is owned by [Player actions and special moves](Player-Actions-and-Special-Moves); versioned results by [Sektor takeover proof history](Sektor-Takeover-Proof-History). Donor addresses in the table are retail VAs.
...
| Palette | Donor `0x8007F75C` binds a projectile-local palette via `0x80001730` with `0x80156A70` (rocket palette source); explosion also uses its own effect resource. | After `0x80034510` returns the new actor, `0x80030974` calls `0x8001C528(0x800B1A24,0x100,0)` and stores `(return - 0x80)` to new actor `+0x9E` **before actor insertion**. This fixed stock record is independent of the constructor resource operand, explaining why v80's descriptor-only change could leave the first visible projectile Ice-blue. On nonzero-mode branches the parent wrapper also stores selector zero; it is bypassed on the ordinary straight shot. The player's resident Sektor source is dynamically allocated through the type-4 fighter source table; selector zero denotes fixed handle `0x80`, not that source. v86 changes only this source ownership and Runtime-confirms a materially more coherent rocket, but not donor-faithful presentation: the existing imported rocket pixels were already quantized into grayscale Sektor TLUT entries. This separates **palette ownership** from **palette content/index translation**. v81 keeps the stock allocation/refcount call but, as a **disposable diagnostic**, forces the token-`0x0B` pre-insertion selector store to `0` while also using the live `+0xDB8/+0xDBC` rocket constructor operand. **Runtime-confirmed partial result:** the one-frame blue Sektor-like projectile fallback disappears, showing first presentation changes; however, the travelling rocket becomes visibly sparse/corrupted. Selector zero names a different palette source, and the exact slot-zero runtime TLUT contents are unobserved. Frame-by-frame review shows the projectile remains a compact horizontal 39x9-class silhouette but only sparse bright pixels survive, consistent with an incorrect palette source; exact pixel attribution still needs a runtime slot/readback. This store override is intentionally broader than the eventual generic adapter and is not a production implementation. |
| Blue helper actors and screen tint | The MKT rocket routine has no Ice companion-actor pair or blue screen tint in this chain. | Ice parent saves original `+0x714/+0x6E4`, installs temporary global actor/script state (`+0x714=*0x80291C10`, `+0x6E4=0xF88`, `+0x718=6`), snapshots owner coordinates into globals, then starts **two separate class `0x100` processes** at `0x8004AE68/84`. `0x80060A3C` allocates a palette/texture, constructs a resource-backed companion actor, inserts **both newly constructed companion actors** into the render list, updates the pair from its own script through `0x80060D14`, removes both and releases the texture at `0x80060E20..44` (no palette release occurs in the traced helper cleanup), then exits. This accounts for the two blue Sektor-like companions. `0x8004AB0C` calls `0x80044964` for blue screen tint and `0x80028564` for process exit; it creates no actor. The blue first projectile frame is a **third, separately owned** image (the Ice descriptor above). These are Ice-only semantics, but helper teardown and parent staging cannot be removed by NOPing creation. |
...
**Reconciled adapter contract (v80–v89).** Keep the layering `MKT move semantics → generic donor→MKMSZ adapter primitives → MKMSZ host ABI`. `create_projectile` acquires its own frame resource, texture slot, source palette handle and selector; `bind_frame` runs after actor context is initialized and establishes descriptor/render state; `prepare_texture_slot` ensures the finalized frame pixels are actually resident before first actor publication when the host would otherwise defer upload; `place_local` uses the confirmed `(+5,+38)` translation without assuming `0x80031394` is safe; `handoff_child` preserves `+0x714 -> +0x6E0` and parent `+0x648` clearing; `flight_tick` preserves the v75 cadence/velocity and separately translates strike, effect, audio and cleanup semantics. Actor-list insertion is part of update and render eligibility, not a substitute for frame binding or resource acquisition.

The parent straight branch (`0x80111F98=0`) calls `0x8004B584` and jumps past `0x8004B040`, so the much-cited v75 parent wrapper is **not** the transition proven by the normal flight capture. The parent `0x8004B5A0` advances an Ice entry after token insertion; the child `0x8004B82C` loads `+0xD90`, and the generic flight timer later advances it to the rocket. The actor `+0x74` same-entry comparison in `0x8001BDA0` is not a skipped rebuild here: both relevant descriptor preceding words are zero, so both changed and repeated binds on the initialized `+0x8C bit 0x40` path increment dirty `+0xFA`. `0x800185B8` consumes that request and queues a texture-slot upload; `0x80024650` only links into the active motion/render list.

This ordering and palette ownership explain the controls together. v80 changes the constructor entry but retains the stock Ice palette and later stock Ice frame. v83's raw early rocket bind is overwritten by the parent's stock frame advance before flight. v81/v82 and v84 force selector zero: v82 additionally synchronizes `+0x80` to the same wrong selector, explaining its unchanged image. v84's full early cursor transaction changes controller `+0x6E4` to `+0xD90`, so the parent advance reads a rocket entry rather than the stock Ice entry; its zero selector is an independent wrong-source change; same-shape poisoning is not supported by the code. The exact per-pixel cause of the captured sparse image requires reading the actual slot-zero TLUT and render job at runtime; no such capture exists. v85's deferred insertion is statically unsound for the ordinary straight route because the only proposed reinsertion is in the **bypassed** parent wrapper. Its global token suppression leaves the projectile off the motion/render list, and normal cleanup `0x80024B00` cannot return an unlisted actor to the free list. v85 is built but never runtime-tested and should not be advanced as a proof.

The v86–v89 sequence now resolves the first-visible rocket contract. v86 proves projectile-local dynamic palette ownership; v87 proves the exact retail ROCKETD1/ROCKET_P asset pair; v88 proves that early descriptor/cursor state alone still leaves one frame of stale slot pixels; v89 Runtime-confirms that synchronously preparing the finalized projectile texture slot before actor publication removes that frame while retaining correct colors and v75 flight. Production work must now replace the one-shot proof allocation/storage recipe with owned, bounded texture/palette lifetime and balanced release. Do not return to selector-zero or cosmetic first-frame tuning.
