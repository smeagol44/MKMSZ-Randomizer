...
| ROM `0x315EC..0x31603` / VA `0x800309EC..0x80030A03` (v82 diagnostic) | Pre-insertion selector-0 active-handle coherence | Replaces the token handler's redundant `actor+0x7C=actor+0x108` rewrite plus surrounding reloads with native table lookup `lhu 0x80290A02 -> actor+0x80`, then restores owner/type/current-controller values needed by the continuation. Constructor `0x800281A0` already performed the removed `+0x7C` assignment. Implementation/static-confirmed; Runtime Pending. |
