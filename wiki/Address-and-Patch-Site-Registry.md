...
| ROM `0x316C8` / VA `0x80030AC8` plus proof ROM `[0x9AE10,0x9AE90)` (v83) | Post-init/pre-insertion genuine rocket bind | v83 redirects the final token actor-list insertion through a 56-byte wrapper that calls native `0x8001BDA0` with `actor+0x98+0x4E6E4`, then tail-jumps to the original `0x80024650`. The existing v75 parent wrapper is compacted into the first 72 bytes of the same proof interval, preserving selector-0/D90/cursor-restore behavior; child code still begins at `0x9AE90`. Implementation/static-confirmed, Runtime Pending. |
