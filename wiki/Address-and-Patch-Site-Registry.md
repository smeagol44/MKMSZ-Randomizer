...
| ROM `0x00016354` / VA `0x80015754` | Direction-facing v10 pre-dispatch Turn-action gate | **Runtime-confirmed bounded proof.** Expected bytes `A62406DC 3C058003` are replaced by a jump to the v10 trampoline + NOP. The helper player-gates via controller `+0x638 == 0x800BF2EE`; states `23`/`24` jump to `0x80015768` before generic action installation, while all other actions reproduce the displaced store/LUI and resume at `0x8001575C`. Semantic Turn/Combine is not cleared. |
| ROM `0x0009AE58` / VA `0x8009A258` | Direction-facing v10 action-gate trampoline | **Runtime-confirmed bounded proof.** Expected 16 zero bytes become an indirect jump to the uncached helper appended to file `0x1A`; proof-only, not production-safe ownership. |
| ROM `0x00F72270..` / RDRAM `0x801AFA90..` | Direction-facing v10 action-gate helper extension | **Runtime-confirmed bounded proof.** v10 appends the pre-dispatch helper at file-`0x1A` offset `+0x270`, extends the file end on a 16-byte boundary, and remains below the reserved expansion-pool ceiling `0x801B3420`. |
...
| ROM `0x315E8` / VA `0x800309E8` (v81 diagnostic) | Token-`0x0B` newly constructed actor selector store before insertion | Exact v75 `A602009E` = `sh v0,0x9E(s0)`; v81 uses `A600009E` = `sh zero,0x9E(s0)` while preserving the preceding `0x8001C528` call. Implementation/static-confirmed, Runtime Pending. Disposable token-handler-wide diagnostic only; not an accepted production/global edit. |
