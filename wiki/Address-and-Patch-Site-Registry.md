...
| ROM `0x0002A658` / VA `0x80029A58`; ROM `0x0002A85C` / VA `0x80029C5C` | Direction-facing v05/v06 attempted direct Turn-toggle suppression | **Rejected / incomplete for the tested route.** Guarded `0x14400007 -> 0x10000007` at the first site (v05) and then both sites (v06) did not stop the visible Turn. These addresses remain useful negative-control evidence, not accepted live Turn patch sites. |
| ROM `0x0003E46C` / VA `0x8003D86C` | Direction-facing v03/v04 attempted Turn suppression | **Rejected / failed for visible player Turn.** Player-gated and unconditional-return variants did not alter the observed turn. |
