...
| ROM `0x0002A658` / VA `0x80029A58`; ROM `0x0002A85C` / VA `0x80029C5C` | Direction-facing v06 dual player-loop Turn suppression | Pending runtime; both guarded stock branch words are `0x14400007` and become `0x10000007`. Their delay slots retain the stock Turn latch write, while both direct actor `+0x8C XOR 0x10` facing-toggle blocks are skipped. v05 patched only the first site and was runtime-incomplete. |
| ROM `0x0003E46C` / VA `0x8003D86C` | Direction-facing v03/v04 attempted Turn gate | **Rejected / failed for visible player Turn.** Gated and unconditional-return variants did not affect the observed turn. |
