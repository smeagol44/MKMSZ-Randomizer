...
| v88 | **Implementation/static-confirmed; Runtime Pending** | Combines v87's dynamic ROCKET_P ownership with v84's proven pre-insertion contextual rocket advance, but does not overwrite the dynamic selector with zero. |
...
- **v88 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-first-frame-rocket_common-proof_v88.z64`, SHA-256 `196899276ef9cbc494714dc60862437402a7a22d7e2af1100f584e653c5dfed0`, CRC1/CRC2 `5F0CC13B / 97B0B76C`. Built from exact v75 + exact MKT Rev. 2. It retains v87's exact-index raw rocket and dedicated dynamic ROCKET_P acquisition, then redirects only the token's final actor insertion through a contextual pre-insertion wrapper equivalent to the v84 first-visible transaction: projectile becomes current, controller `+0x6E4` is advanced from `actor+0x98+0xD90` through full `0x800304C0`, then the saved cursor/owner are restored and stock `0x80024650` insertion proceeds. Crucially, v88 does **not** write selector zero; the v87 dynamic ROCKET_P selector remains intact. The ordinary-straight test route remains `Forward, Forward + Low Punch`; palette release remains Pending, so begin with one shot.
