...
| v81 | **Implementation/static-confirmed; Runtime Pending** | Paired pre-insertion rocket constructor + selector-0 diagnostic; preserves v75 machinery and awaits manual test. |
...
- **v81 — Implementation/static-confirmed; runtime Pending.** Disposable proof `MKMSZR_sektor-missile-first-render_common-proof_v81.z64`, SHA-256 `424b6ddb229f9c3f7e775212357ca3e862ed75c343fb14b8f44de233b4c0f607`, CRC1/CRC2 `29C9C3E6 / E9E88C41`. It keeps exact v75 placement, child ownership, cadence/acceleration, companion/tint processes, collision and cleanup. Relative to v75 it applies the validated v80 constructor operand and changes only token-handler VA `0x800309E8` / ROM `0x315E8` from `sh v0,0x9E(s0)` to `sh zero,0x9E(s0)`, forcing the newly constructed actor onto selector 0 **before insertion** while preserving the preceding `0x8001C528` allocation/refcount call. This token-handler-wide selector override is deliberately diagnostic, not the final generic adapter. Manual runtime input remains `Forward, Forward + Low Punch`.
