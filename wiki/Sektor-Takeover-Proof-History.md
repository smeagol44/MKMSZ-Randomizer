...
| v83 | **Implementation/static-confirmed; Runtime Pending** | Keeps stock constructor/palette; performs the genuine rocket frame bind only after token initialization and immediately before insertion. |
...
- **v83 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-postinit-bind_common-proof_v83.z64`, SHA-256 `e15a48d019a868ac542c41c82558ff7270aea0666eebc007d53a5a4bdafdb23d`, CRC1/CRC2 `9DEC7AB0 / D63748B1`. Built from exact v75. It keeps the stock Ice constructor operand and stock token palette allocation/selector, then redirects only the final token actor-list insertion call through a 56-byte wrapper that binds `ROCKETD1` with native `0x8001BDA0` **after** token actor initialization and immediately **before** the original `0x80024650` insertion. The existing v75 parent wrapper is compacted but preserves its selector-0, `+0xD90`, one-advance, cursor-restore behavior. The two routines reuse exactly the existing proof interval `[0x9AE10,0x9AE90)`; no new allocation is claimed. Placement, child ownership, calibrated flight, companion/tint processes, collision and cleanup remain v75 controls.
