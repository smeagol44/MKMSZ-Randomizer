...
| v84 | **Implementation/static-confirmed; Runtime Pending** | Moves the full known-good v75 contextual `+0x6E0/+0x6E4 -> 0x800304C0` rocket-frame transaction to the pre-insertion seam. |
...
- **v84 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-context-bind_common-proof_v84.z64`, SHA-256 `0d51810737fdf04e73b27dd0b00a1c082588349d6cf907aef6c09d0a57aed90b`, CRC1/CRC2 `9CF5BA48 / 98A4A465`. Built from exact v75. It preserves the stock constructor operand and stock token selector, but factors the later Runtime-confirmed v75 first-frame transaction into a shared helper: save controller cursor, make the projectile current at `+0x6E0`, set selector 0, set `+0x6E4` to projectile resource base `+0xD90`, call full native animation interpreter `0x800304C0`, restore cursor; at the insertion seam the shim then restores owner `+0x6E0` and tail-calls original `0x80024650`. The existing later v75 hook calls the same helper. The exact proof interval `[0x9AE10,0x9AE90)` is reused; child/flight code from `0x9AE90` onward is byte-preserved.
