...
- **v79 — Runtime Pending.** Built from exact v75, not v76-v78. It composes several statically resolved changes in one bounded proof: (1) the single genuine chest-open shape becomes an eight-entry reduced animation consumed by the existing stock animation-advance sequence; (2) the six post-launch stock sleeps are resampled to total `0x20` target ticks; (3) the projectile inherits the owner Sektor palette selector before native frame binding instead of forcing selector 0; (4) only the inherited Ice blue-screen tint call is suppressed while its helper process exit remains; (5) fighter-type-4 projectile strike selector `0x13` keeps the target-native hitbox but changes unblocked reaction `0x2C -> 0x04` and damage `0/0 -> 20/2`. v75 flight and `(+5,+38)` placement are byte-preserved. Clone-helper actors, smoke, impact explosion, and imported donor audio remain intentionally Pending.
...
For contact, fighter type 4 uses strike-pointer table `0x800AF484`; selector `0x13` resolves to record `0x800AF634`, whose stock bytes encode the Ice projectile hitbox, unblocked reaction `0x2C`, blocked reaction `0x05`, zero normal/block damage, and flags. v79 deliberately changes only reaction/damage semantics while retaining the target hitbox and generic `0x8004CC50 -> 0x8004CE6C/0x8004CF3C -> 0x8002BA04` collision path. Reaction `0x04` is reused because v62 Runtime-confirmed it as the safe target-side replacement for the tested hard-hit combo reaction; exact equivalence to donor `r_rocket` remains Runtime Pending.
