...
| v82 | **Implementation/static-confirmed; Runtime Pending** | Adds the native selector-0 active-handle rebind before insertion while preserving the rest of v75. |
...
- **v82 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-palette-bind_common-proof_v82.z64`, SHA-256 `becf7a1e6bc1276976d844105fda42d07bc5af54d3be4fca92324811092c442c`, CRC1/CRC2 `0A70C1B2 / 72AEDC2B`. Built from exact v75, it keeps the v80 rocket constructor and v81 selector-0 store, then mirrors native `0x8001BDA0`'s selector-0 active-handle lookup (`0x80290A02`) into actor `+0x80` before insertion. To fit this in place it removes only the token handler's redundant `actor+0x7C = actor+0x108` rewrite; constructor `0x800281A0` already performed that exact assignment before returning. Placement, child handoff, v75 calibrated flight, both Ice helper processes/tint, collision and cleanup are otherwise byte-preserved. Manual input remains `Forward, Forward + Low Punch`.
