...
The proof line now continues through **v82** for Sektor's straight missile. v75 is the stable Runtime-confirmed flight baseline; v76/v77 did not solve presentation/placement, v78/v79 hard-hung (their individual causes are not established), v80 is a stable but visually ineffective constructor-only proof, v81 Runtime-confirms that constructor-time rocket substitution removes the one-frame blue projectile fallback but leaves the missile graphically corrupted; v82 produces no visible change relative to v81, rejecting stale `+0x80` alone as the explanation. Post-v82 static reconciliation identifies the deeper seam: `0x800281A0` runs native frame setup before token `0x0B` finishes secondary-actor initialization, while exact v75 performs a later full Ice→rocket shape transition. v83 moved the rocket bind to the fully initialized pre-insertion boundary, but runtime screenshots show no material improvement: the first visible projectile frame is still the Ice/Sektor-like fallback and the following missile remains Ice-blue. This rejects a raw `0x8001BDA0` post-init/pre-insertion bind as sufficient. Static reconciliation now isolates the additional host context used by the known-good later v75 bind: projectile-as-current actor plus temporary `+0x6E4 -> +0xD90` and a full `0x800304C0` advance. v84 Runtime-confirms that the full contextual pre-insertion bind removes the one-frame Ice/Sektor projectile fallback, but the missile still renders with the same sparse/glitched rocket texture seen in v81. This establishes that first-visible timing can be controlled there, but binding ROCKETD1 before the original v75 bind poisons the later same-shape update. v85 therefore leaves the actor on its stock Ice descriptor and **defers only active-list insertion** until immediately after the original v75 Ice→rocket bind; v85 is Runtime Pending. v63 is rejected because an assumed-dead file-`0x87` bank was actually live during hit/blood presentation. v64 established a compact one-chest/one-missile asset composition and reached the special path, but exposed that the reused Ice presentation spawned duplicate blue/frozen Sektor actors and retained Ice-like flight. v65-v67 are rejected immediate-hang lifecycle experiments. v68 is a stable negative-control build with stock Ice lifecycle restored but no chest substitution. **v69 Runtime-confirms the narrow player-pose hook:** current controller/process `+0x6E4` can be redirected after the stock helper and advanced once to display the genuine chest-open frame without hanging. **v70 is partially Runtime-confirmed:** the genuine rocket frame appears and travels, but stock Ice helper clones, blue tint, distant spawn, and ineffective/incorrect flight translation remain. v70 therefore validates presentation/host-path pieces, not a completed or faithful missile port. v71 is **Rejected / failed** after a 1-3-frame hard hang; its post-test trace confirms that stock `+0x648` placement and `+0x714` child handoff refer to the same secondary actor, while its per-tick reuse of `0x8004CC14` is a static semantic mismatch. **v72 Runtime-confirms the isolated placement translation:** direct native local `(+5,+38)` spawns the rocket near Sektor and mirrors correctly across facing. **v73 Runtime-confirms stable per-tick acceleration through projectile actor `+0x14` without hanging**. **v74 Runtime-confirms the cadence-resampled movement path is stable, but comparative donor/target video shows its visible velocity magnitude remains under-scaled by about 2× on the tested route.**
...
| v81 | **Partial Runtime-confirmed** | Removes the one-frame blue projectile fallback, but selector-only rebinding corrupts the rocket; proves the first-display selector seam and exposes stale active handle `+0x80`. |
...
| v85 | **Implementation/static-confirmed; Runtime Pending** | Keeps stock Ice state off-list, runs the original v75 Ice→rocket transition at its proven point, then performs the deferred stock list insertion. |
...
## v75-v81 — calibrated flight and first-render reconciliation
...
- **v84 — Partial Runtime-confirmed first-visible ownership result.** Disposable proof `MKMSZR_sektor-missile-context-bind_common-proof_v84.z64`, SHA-256 `0d51810737fdf04e73b27dd0b00a1c082588349d6cf907aef6c09d0a57aed90b`, CRC1/CRC2 `9CF5BA48 / 98A4A465`. Runtime screenshot shows the initial one-frame Ice/Sektor projectile fallback is gone, proving the contextual pre-insertion bind is early enough to control first visibility. The following missile remains the same sparse/glitched rocket texture as v81. Because the later v75 wrapper then sees the same ROCKETD1 shape instead of the original Ice→rocket transition, v84 is not a valid final binding strategy. Evidence screenshot is archived as `MKMSZR_SEKTOR_MISSILE_V84_RUNTIME_GLITCHED_ROCKET_2026-09-25.png`.
- **v85 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-deferred-insert_common-proof_v85.z64`, SHA-256 `23bc0cd6d677fc99805cdb6bdeebe160073b18ac1f71b8855d87fbf3562f30ea`, CRC1/CRC2 `B52962D5 / 07C23284`. Built from exact v75. It does **not** bind the rocket early. Instead it suppresses only token handler `0x80030AC8`'s call to `0x80024650` while preserving the delay-slot `actor+0xAE` store and all staging pointers. The actor remains stock-Ice and unlisted through the existing `(+5,+38)` placement. At the original Runtime-confirmed v75 wrapper point, the normal full Ice→ROCKETD1 `0x800304C0` transition runs first; the wrapper then calls the original `0x80024650` to make the already-rocket actor visible. `0x80024650` is only a sorted list link; to preserve its stock sort key, v85 temporarily substitutes owner Y during insertion and restores the projectile's final placed Y immediately afterward. Child code, flight, helper/tint processes, collision and cleanup remain byte-preserved. The token-call suppression is proof-global and therefore bounded to this manual missile route, not a production implementation.
...
The blue presentation has two distinct inherited sources. The projectile frame wrapper had been forcing actor palette selector `+0x9E = 0`; target frame setup resolves the active handle from that selector, so v79 instead copies the already-correct owner selector before binding the genuine rocket frame. Separately, the second stock Ice helper callback at `0x8004AB0C` is presentation-only around `0x80044964(128,128,192,128,...)` followed by native process exit; v79 suppresses only that tint call. The two visible Ice/Sektor clone actors come from the separate `0x80060A3C` helper path and are not removed in v79.
...
Clean donor/target plus v75 disassembly resolves the key process distinction: Ice parent starts class-`0x100` companion constructor `0x80060A3C` and tint callback `0x8004AB0C`; the former allocates texture/palette, constructs/inserts and later removes two actors, while the latter calls screen tint and exits. The separate class-`0x700` projectile child is created through `0x8004CBC4`. The parent stores temporary actor/script/coordinate state before the helpers, restores its staged projectile pointer, then hands that actor to the projectile child. The initial blue projectile frame is a third source: the token inserts an initially Ice-derived actor before the v75 parent wrapper binds a rocket frame; the child later installs an Ice cursor. Full donor/host semantics are in [MKT adapter primitives](MKT-Adapter-Primitives).

v65-v67 suppressed one or two process spawns **and** altered parent animation/setup/movement. Restoring one companion in v67 changed visuals but did not restore the complete transaction. v78's direct `0x80031394` shim altered position/anchor/facing at a pre-rocket-descriptor seam and hard-hung. v79 changed five independent concerns and hard-hung. None identifies a single causal instruction. v80 then resolved and changed the exact live constructor operand at file-`0x87 +0xDBC`, but the visible Ice-derived projectile presentation remained. Static reconciliation explains why: after `0x80034510` constructs the actor, token handler `0x80030974` separately calls `0x8001C528(0x800B1A24,0x100,0)`, stores `return-0x80` to actor `+0x9E`, and only then inserts the actor. Exact v75 also has `+0xFD0 -> +0xD90`, where `+0xD90` is already the genuine rocket loop, so the child cursor is not the remaining Ice-descriptor seam. The smallest next proof is therefore the v81 paired first-render transaction above: preserve all v75 lifecycle/flight machinery, retain the v80 rocket constructor operand, and force selector 0 before insertion. If this removes the initial Ice-blue projectile frame while companions/tint remain, the constructor+palette boundary is validated; if not, trace active handle `+0x80` and palette lifetime before changing any further presentation behavior.
