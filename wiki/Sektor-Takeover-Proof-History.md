...
## v75-v79 — calibrated flight and presentation/contact composition
...
### Post-v79 ownership reconciliation
...
v65-v67 suppressed one or two process spawns **and** altered parent animation/setup/movement. Restoring one companion in v67 changed visuals but did not restore the complete transaction. v78's direct `0x80031394` shim altered position/anchor/facing at a pre-rocket-descriptor seam and hard-hung. v79 changed five independent concerns and hard-hung. None identifies a single causal instruction. The smallest next proof should retain the v75 parent, both helper processes, placement, acceleration and strike path while changing only the token-`0x0B` **resource actor constructor/first descriptor** so the staged actor has the rocket image before insertion. Leave the existing later parent wrapper, child entry, palette, collision and cleanup as controls. Trace the exact entry consumed by `0x80034510`, prove its reference scope, and guard its descriptor/pointer before a ROM build. That would validate a reusable constructor-time binding boundary rather than another visual offset.
