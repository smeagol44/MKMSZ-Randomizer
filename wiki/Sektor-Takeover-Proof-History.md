...
| v87 | **Implementation/static-confirmed; Runtime Pending** | Uses exact retail ROCKETD1 indices `0..15` plus exact retail ROCKET_P converted to a dedicated target palette source; v75 frame/lifecycle/flight remains the control. |
...
- **v87 — Implementation/static-confirmed; Runtime Pending.** Disposable proof `MKMSZR_sektor-missile-rocket-palette_common-proof_v87.z64`, SHA-256 `2137de34fc34b29a0d9c0f62164a6961714cb27b464824d5e354756655885b9e`, CRC1/CRC2 `4C35F4F0 / DCDBD00C`. Built from exact v75 plus exact MKT Rev. 2 donor SHA-256 `30efdbe2…`. Static donor verification finds retail `ROCKET_P` at ROM `0xBC3AC` (count 16; colors at `0xBC3B0`) and ROCKETD1 at robot-relative `+0x2668`, codec-16, `39x9`, anchor `(+26,+3)`, 360 row-aligned decoded bytes using all indices `0..15`. v87 appends a target raw/type-0 copy at file-`0x87 +0x4E790`, appends a 64-entry target palette record at `+0x4E8FC` (source `+0x4E900`), redirects the existing ROCKETD1 descriptor image pointer to the raw copy, and retargets only the guarded straight-token palette source through native `0x8001C528`. File-`0x87` grows `0x4E790 -> 0x4E980` (`+0x1F0`). Stock Ice constructor/frame schedule/insertion, helper/tint processes, `(+5,+38)` placement, child handoff, v75 flight/collision and cleanup are preserved. Palette release remains unproven; manual validation should begin with one neutral straight shot.
