...
| `0x80028610` | Main process scheduler/dispatcher | Static-confirmed | Runs once per gameplay main-loop iteration on the normal route; decrements controller sleep `+0x6DA` by elapsed retrace count `0x802FCD44 + 1` on the ordinary path, context-switching runnable controllers through `0x8007CAF8`. Main gameplay later waits for `0x802FCD44 >= 2`, yielding the established ~30-Hz ordinary gameplay scheduler cadence. |
| `0x80028794` | Process sleep/yield | Static-confirmed | Stores requested sleep at current controller `+0x6DA` and context-switches through `0x8007CAF8`; ordinary Run calls it with `1` before each animation-rate advance. |
| `0x8002FE54` | Fighter animation-table selector | Static-confirmed | Argument `a0=0` indexes file-relative table 0 at `+0x000`; nonzero `a0` adds `0x104` and indexes table 1. No bounds check is performed. Because MKMSZ table 0 has 65 entries, table-0 index `0x41+n` aliases table-1 index `n`; stock code intentionally uses this aliasing. |
| `0x8002EB38` | Ordinary Run setup | Static-confirmed | Selects primary animation slot `0x2B` through `0x8002FE54`, installs animation rate `2` through `0x80031724`, then applies facing-aware horizontal velocity. Run loop caller at `0x800296B0` sleeps one process tick and invokes `0x8003174C` once per active iteration. |
| `0x80031724` | Animation-rate initializer | Static-confirmed | Stores period argument at controller `+0x6A6`; primes countdown `+0x6A8` to `1` (or `0xFFF` for never-advance). Ordinary Run passes `2`; cadence audit identifies `3` as the donor-equivalent Sektor Run period on the ~30-Hz target scheduler. |
| `0x8003174C` | Rate-gated animation advance | Static-confirmed | Decrements controller `+0x6A8`; when zero, reloads period from `+0x6A6` and advances the current script through `0x800304C0` or `0x800317C8` according to actor flags. Ordinary Run calls it once after each `process_sleep(1)`. |
...
| `0x8003D86C` | Native turn-action routine | Static-confirmed | Selects **secondary table-1 slot `0x03`** (`file 0x87 +0xE84`) and runs the stock turn-action sequence through the current controller; the older primary-`0x03` description was incorrect. Not intrinsically player-only. |
...
| `0x800304C0`, `0x80030B6C` | Shared animation advancement and token-`0x10` paired-actor handler | Static-confirmed | On the player path, advancing off token `0x10` marks an existing `+0x650` companion `+0x64 bit 0x20` (hidden), without unlinking it. Token `0x10` creates and links a companion once when absent/flagged, or reuses it, clears its hidden bit, copies existing owner resource/palette/texture bindings, aligns it, and binds consecutive companion/owner descriptor operands. The shared handler is not a player-only patch seam. Detailed ownership and unresolved stage teardown: [Sub-Zero to Sektor animation mapping](Sub-Zero-to-Sektor-Animation-Mapping). |
...
| `0x80060A3C -> 0x80060D14` | Ice companion-actor constructor / per-process update | Static-confirmed | Class-`0x100` helper allocates texture/palette, constructs and inserts two visible actors, advances and copies script frames; removes both and releases texture (no palette release in traced cleanup) at `0x80060E20..44` before process exit. Parent staging coordinates/script/secondary pointer are inputs, so its spawn is not a pure visual switch. |
| `0x8004B82C` | Ice-projectile flight callback | Static-confirmed | On straight mode zero loads file-`0x87 +0xFD0 = +0xD90` rocket cursor, calls one-time `0x8004CC14`, then v75 injects initial velocity and flight callback; the parent wrapper is bypassed |
| `0x8004B584` | Parent preflight actor frame/strike check | Static-confirmed | Makes `+0x714` current and calls stock `0x800304C0`, consuming Ice entry `+0xDC4` before child flight on normal mode zero; restores owner before strike/handoff |
| `0x8001BDA0` | Actor descriptor/render-record binding | Static-confirmed | Writes `+0x74/+0x7C` descriptor geometry; initialized `+0x8C bit 0x40` path marks `+0xFA` dirty and resolves `+0x68/+0x80` from texture slot `+0x9C` and palette selector `+0x9E`; both relevant zero-preword entries mark dirty even on same-shape binds |
| `0x800185B8`, `0x8001D6EC`, `0x8001D7F0` | Deferred image decode and texture upload | Static-confirmed | Active render/update pass consumes `+0xFA`, decodes `+0x7C`, queues slot upload, then frame dispatcher drains queue; `0x80024650` merely links into that active movement/render list |
| `0x8001C528`, `0x8001C64C` | Palette source acquisition and owned handle release | Static-confirmed | Lookup/allocate returns handle; actor stores handle minus `0x80` at `+0x9E`; release takes original handle. Stock token acquires fixed Ice source, player type-4 acquires file-`0x87 +0x4584C` Sektor source through fighter table. Selector zero is handle `0x80`, not the player palette |
