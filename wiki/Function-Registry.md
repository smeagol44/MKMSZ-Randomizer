...
| `0x8001F7A8` | Gameplay textured-node renderer | Static/runtime-confirmed | Used by the gameplay HUD queue. Node `+0x4A` is texture-slot ID (v09/v10 runtime-confirmed); node `+0x4C` is palette selector, and v16 runtime-confirms clone-local selector changes take effect. Palette selector resolves through `0x80290A00 + id*8` to a palette index, then through pointer table `0x802E73E0 + index*4` to an already hardware-ready CI8 RGBA5551 TLUT. MKMSZ source BGR555 palette words must pass through source-to-hardware conversion before direct use here. v15 runtime-confirms a dynamic `0x200..0x2FF` texture slot can render genuine Toasty pixels through this path. |
