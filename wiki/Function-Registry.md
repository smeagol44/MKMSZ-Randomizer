...
| `0x80015754` boundary | Generic gameplay action installation seam used by control-facing v10 | Static/runtime-confirmed at the bounded proof seam | Stock displaced words store requested action/state to controller `+0x6DC` then begin generic dispatch setup. v10 player-gates states `23` (standing Turn) and `24` (crouched Turn) before installation, resuming suppressed actions at `0x80015768` and all other actions at `0x8001575C`. |
