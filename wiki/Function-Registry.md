...
| `0x8001BF70` | Gameplay HUD texture-slot initializer | Static-confirmed | Called six times from `0x8005BFB0` after image decode; initializes fixed HUD texture slots `0x11..0x16` in the same 16-byte slot/backing-pointer tables later referenced by textured 0x58-byte HUD nodes; v11 tests one additional proof slot `0x17` with identical stock content |
