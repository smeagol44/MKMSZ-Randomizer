...
**Full frontend proof v02 — Rejected / unsafe.** Preserving `ra` fixed the immediate menu-entry hang, and the four-row layout rendered. Runtime testing then exposed three defects: cursor arrows were vertically misaligned with the compact rows, JUMP could become enabled but appeared unselectable, and menu music could accelerate with crunchy distortion. Static reconciliation found the selector was publishing logical indices `0..4` into the stock GAME SETTINGS cursor contract, whose type-2 coordinate table has only four entries (`0..3`). Logical JUMP therefore reused the old EXIT cursor coordinate, while logical EXIT index 4 read beyond that table. Because the audio symptom occurred in the same unsafe build, v02 is rejected rather than treated as a cosmetic-only issue.

**Full frontend proof v03 — Runtime Pending.** Keep logical rows `0..4`, but patch the stock type-2 cursor table to the compact row Y positions and publish only valid cursor contracts: logical rows `0..3` use type 2 with the same index, while logical EXIT `4` maps to stock OPTIONS cursor type 0, index 5. This should also make JUMP visibly selectable. The accelerated/crunchy music is a required regression check; the cursor overrun is the strongest current suspect, not yet runtime-proven as the audio cause.
