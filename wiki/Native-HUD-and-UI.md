...
**v05 — Runtime Pending boss-fallback proof.** Rebase on stable v03 and remove the broad class-`2..5` scanner from the LOCK paths. Instead, use the stock opponent-specific route: call `0x80031D00` to resolve the nearest opponent controller (classes 2/3 only), then inspect only that controller's `+0x6BC & 0x0200`. If set, gameplay temporarily follows stock/`TOGGLE` semantics without changing the saved `LOCK` menu value. The Temple checkpoint/pickup/reversal route is the regression test; the first Scorpion/type `0x12` encounter is the boss-fallback test. Type `0x12`'s stock characteristics word is `0x0300`, so its `0x0200` face-policy bit is present. Earth remains explicitly Pending because type `0x19` is not statically proven to use this generic flag.
