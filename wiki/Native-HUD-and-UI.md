...
**v06 design — Static-confirmed / build Pending.** Rebase again on stable v03. Do not call `0x8004A6E8`, `0x80031D00`, or any other native helper from the release/action hooks. If boss fallback is retained there, derive the established face-policy predicate directly from the controller list used by stock `0x80028510`: list head `0x80111EAC`, node link `+0x00`, class `+0x6D8`, current-controller exclusion through `0x802ECE20`, and policy bit `+0x6BC & 0x0200`. Use a hard iteration bound so a malformed/transient list cannot spin forever. The stable v03 decision helper remains unchanged.
