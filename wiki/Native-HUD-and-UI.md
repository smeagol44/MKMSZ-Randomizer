...
**v04 — Rejected / failed.** The broad `0x8004A6E8` controller-class scanner was added to all three LOCK paths. A deterministic Temple regression was then isolated: after the bottom-floor checkpoint, picking up the nearby item and immediately reversing direction under `TURN: LOCK` hard-hangs every time. The same route could not be reproduced on v02 or v03 after repeated attempts, so v04 is rejected as the introduction point.

**v05 — Rejected / failed.** Replacing the broad scanner with direct calls to nearest-opponent helper `0x80031D00` did not fix the Temple regression: the checkpoint + pickup + reversal route still hard-hangs. v05 also introduced a second deterministic hang where pressing Action/A in the final upper-floor Temple room can stop the game even with no nearby enemy or pickup interaction. The common structural change shared by v04/v05 but absent from stable v03 is that the release and generic action-install hooks now perform nested native function calls from fragile mid-function seams. Those calls are therefore not accepted from these hooks.

**v06 — Runtime-confirmed on the bounded validation routes.** Rebased on stable v03. The decision helper is byte-for-byte the stable v03 logic. The release and action-install hooks make **no native function calls**: they derive the face-policy predicate directly from the stock controller list (`0x80111EAC`, link `+0x00`, class `+0x6D8`, exclude current `0x802ECE20`, policy `+0x6BC & 0x0200`) with a hard 64-node bound. Static inspection confirms zero `jal`/`jalr` instructions in those two hook bodies. Runtime validation confirmed both prior Temple regressions are gone: the checkpoint + pickup + reverse route is stable, and the upper-floor Action/A hang does not reproduce. The first Scorpion/type `0x12` encounter also correctly forces temporary vanilla/`TOGGLE` semantics while the GAME SETTINGS value remains `LOCK`; LOCK resumes outside the forced-facing encounter. Earth remains Pending because its custom type-`0x19` route is not proven to use the generic policy flag.
