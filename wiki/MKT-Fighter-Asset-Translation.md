...
**v81 first-render boundary (Runtime-confirmed partial):** the genuine `ROCKETD1` constructor plus selector-0 pre-insertion store removes the prior one-frame blue projectile fallback, but the travelling rocket becomes sparse/corrupted. Static trace explains the split: constructor `0x800281A0` invokes `0x8001BDA0` before token handler `0x80030974` later rewrites actor `+0x9E`; `0x8001BDA0` resolves `+0x9E` to active palette handle `+0x80`. v81 therefore proves that descriptor + selector are necessary but not sufficient; `+0x9E/+0x80` must be rebound coherently before first display. v82 is the bounded pending proof that writes the selector-0 native handle from `0x80290A02` into `+0x80` before insertion while keeping the stock allocation/refcount call and v75 lifecycle machinery.
