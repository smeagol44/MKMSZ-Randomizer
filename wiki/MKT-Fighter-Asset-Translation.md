...
**v81/v82 first-render boundary:** v81 removes the prior one-frame blue projectile fallback but leaves the rocket sparse/corrupted; v82 explicitly rebinds selector-0's native active handle into actor `+0x80` and produces no visible change. The active-handle-only hypothesis is therefore rejected. Static tracing instead shows that constructor `0x800281A0` invokes `0x8001BDA0` before token `0x0B` finishes initializing the new actor. Since `0x8001BDA0` compares old `+0x74` against the requested shape, early constructor-time ROCKETD1 makes the later v75 bind a same-shape update. v83 returns the constructor to stock Ice and performs the first genuine rocket frame setup only after the token has assigned its owner-derived state and immediately before insertion. Palette translation remains a separate later concern.
