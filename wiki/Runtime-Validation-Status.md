...
| Reverse Elbow / MKT port | v6 host-action lifecycle repeats without global hang; A1/A1.1 genuine SCCOMBO10 resource path renders deterministically, and A1.1 produces a coherent donor-frame silhouette after scan-order conversion | Corrected host scheduler/action paths; exact retail donor strike/reaction/resource closure; MKT codec-22/24 decode; donor X-major -> MKMSZ row-major conversion; isolated native palette path mapped | A1.2 isolated-palette runtime validation; full 10→11→12→11→10 animation; no-repel shim; translated donor strikes/reactions; completed move port |
