...
| Reverse Elbow / MKT port | v6 host-action lifecycle repeats without global hang; v08/A1.7 renders genuine SCCOMBO10; Sektor v08 runtime-confirms the genuine five-frame stance can coexist with forward movement, crouch, and airborne-forward drift without hangs once the false cave is restored; colors are intentionally wrong in v08 because donor palette handling is disabled | The v01-v07 hard hangs are runtime-attributed to overwriting zero-filled live action/dispatch records at `0x800A076C..`; v09 re-homes the corrected palette helper/state into the explicitly reserved `0x801AF420..0x801AF81F` proof runtime block | Runtime validation of Sektor v09 correct colors + stable locomotion; then isolate/fix the remaining one-frame transition artifact; full donor animation and move semantics remain later work |
