...
| Reverse Elbow / MKT port | v6 host-action lifecycle repeats without global hang; v08/A1.7 renders genuine SCCOMBO10; Sektor v01-v06 runtime-confirm the genuine five-frame Sektor stance itself, but the same forward/crouch/air-forward hangs persist through v06; v04's pre-game hang was caused by its added diagnostic HUD | Corrected host scheduler/action paths; exact donor resource closure; codec conversion; native-cursor and donor-rate-hook hypotheses rejected; v07 bypasses the dynamic donor-palette/select-animation wrapper entirely while keeping imported Sektor frames | Runtime validation of Sektor v07; determine whether transition corruption and hangs share the palette/render-state cause; full donor animation and move semantics remain later work |
