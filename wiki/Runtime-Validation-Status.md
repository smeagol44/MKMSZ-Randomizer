...
| Reverse Elbow / MKT port | v6 host-action lifecycle repeats without global hang; v08/A1.7 renders genuine SCCOMBO10; Sektor v09 runtime-confirms correct genuine Sektor colors plus stable forward/crouch/air-forward movement, attacks, block, turning, and jumping with helper/state re-homed into owned runtime memory; one corrupt transition frame remains | The v01-v07 hangs are runtime-attributed to overwritten live zero dispatch records; v10 restores `select_animation` to stock and moves palette switching to native frame setup so shape and active palette change atomically | Runtime validation of Sektor v10 transition cleanup; donor idle cadence 8 still not re-enabled in this proof; full donor animation and move semantics remain later work |
