...
from mkmszr.patches.generalized_persistence import (
    CAPTURE_HELPER,
    CAPTURE_HELPER_ROM,
    CAPTURE_HELPER_VA,
    DESCRIPTOR_TABLE,
...
def test_generalized_code_fits_confirmed_regions() -> None:
    assert len(GENERALIZED_PAYLOAD) == CODE_SIZE == 0x200
    assert GENERALIZED_ACTUAL_CODE_SIZE == 0x1D4
    assert hashlib.sha256(GENERALIZED_PAYLOAD).hexdigest() == (
...
def test_generalized_patch_installs_shared_hooks_and_data() -> None:
    rom = _clean_shape()
    ...
    assert [result.name for result in results] == [
        ...
        "generalized-pickup-persistence-experiment",
    ]
